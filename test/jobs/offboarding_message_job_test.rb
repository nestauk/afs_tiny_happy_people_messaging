...
class OffboardingMessageJobTest < ActiveSupport::TestCase
  ...
  test "#perform sends offboarding message in user's preferred language" do
    ...
    User.any_instance.stubs(:survey_link_token).returns("ABC")

    stub_successful_twilio_call("Diolch yn fawr iawn am gymryd rhan yn ein rhaglen negeseuon testun gyda gweithgareddau wythnosol hwyliog i’ch plentyn! Dywedwch wrthym sut rydych chi wedi cael y rhaglen yma: http://localhost:3000/surveys/#{survey.id}/edit?token=ABC", user)
