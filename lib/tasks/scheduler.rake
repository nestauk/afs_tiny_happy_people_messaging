namespace :scheduler do
  ...
  task send_morning_message: :environment do
    User.not_finished_content.contactable.with_preference_for_day(Date.today.wday).wants_morning_message.find_in_batches do |users|
  ...
  task send_afternoon_message: :environment do
    User.not_finished_content.contactable.with_preference_for_day(Date.today.wday).wants_afternoon_message.find_in_batches do |users|
      SendBulkMessageJob.perform_later(users.to_a, :weekly_message)
    end
  end
  ...
  task send_evening_message: :environment do
    User.not_finished_content.contactable.with_preference_for_day(Date.today.wday).wants_evening_message.find_in_batches do |users|
      SendBulkMessageJob.perform_later(users.to_a, :weekly_message)
    end
  end
  ...
  task send_no_timing_preference_message: :environment do
    ...
    User.not_finished_content.contactable.with_preference_for_day(Date.today.wday).no_hour_preference_message.find_in_batches do |users|
      SendBulkMessageJob.perform_later(users.to_a, :weekly_message)
    end
  end
